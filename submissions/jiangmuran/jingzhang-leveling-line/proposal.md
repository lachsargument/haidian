...
| 为什么用在这两条赛道 | 因为读数错了会伤到人。**实测**机器人 14/338（4.1%，八条中最薄且不足次薄一半）、AI公共服务 30 份 | `visual/assets/field_map.json`，普查脚本可重跑 |
| 空间上做了什么 | 一条主脊 9,443 m、8 个分级水准点、3 处重点区、六类用地完整剖分（无重叠无缺口） | 九个 GeoJSON 图层 + `node visual/assets/verify.js` 独立重算全部一类指标 |
...
**本方案的定位声明。** 城市AI治理是本方案的**方法层**，不是卖点——把治理协议本身当成成果，是这次征集里最饱和的做法（最近一次实测：338 份已合入方案中 221 份申报治理赛道，「证据链/可复算」类表述出现在 31.1% 的方案里 [source:FIELD-CENSUS-2026-08]）。本方案把治理当工具，把工具用在覆盖最薄的赛道上：`robotics-autonomous-mobility` 按标签计 338 份中仅 **14 份（4.1%）**，是八条赛道里最薄的一条，且不足次薄者的一半；`ai-public-services` **30 份（8.9%）**，仅次于它。这不是为了避开竞争而选空位——恰恰相反，闭合差机制在这两条赛道上才真正不可替代，因为只有在这里，一次未被复核的错误读数会落在具体某个人身上。
...
**本方案额外自采一组公开数据，并把它作为交付物之一** [source:FIELD-CENSUS-2026-08]。本方案编写了一台可重跑的普查仪器：枚举 GitHub git tree 得到全部已合入方案目录，逐份抓取其公开 `proposal.md` front matter 与 `agent.json`，再汇总统计。最近一次运行（2026-08-09）覆盖 **338 份**，抓取成功 338/338、失败 0；此前四次分别覆盖 184、215、228、298 份。

普查刻意不读 `submissions-data.js`——那是生成的展示索引，会滞后。**这条观察在本方案自己的复测下发生了反转，如实记录：** 8 月 8 日它只列 184 份，比仓库实际少 44 份（19.3%），且两次普查之间条目数完全未变，当时判断「滞后在扩大」；8 月 9 日复测为 292/298，滞后收窄到 **6 份（2.0%）**，当时写成「索引显然已被追平」。**同日第五次复测为 309/338，滞后又回到 29 份（8.6%）——因此「已被追平」同样是一次快照结论，也必须收回。** 四次索引读数为 44 → 31 → 6 → 29 份：它既不在单调扩大，也没有被一次性追平，而是**随合并批次起伏**。本方案不据此邀功，也不据此指责——无法证明因果；写在这里是因为，一个只在对自己有利时才复测的机制不成立，而**连续两次拿单次快照下结论，说明这个毛病不是别人的**。**结论中仍然成立的那部分是方法层面的：评审仪器必须读权威源（git tree），而不是读一个可能滞后、且滞后幅度本身会变的派生索引。**
...
这不是评论主办方的工作，而是**交付一件他们此刻最缺的东西**。截至最近一次实测仓库已合入 338 份方案，PR 编号已过 1000，维护者在以每分钟数份的速度合并。此时最难的不是收稿，是**横读**：三百多份里哪些已经收敛、哪些位置还空着、哪些声明其实无法被聚合。这件事无法靠翻展示页完成，需要一台仪器。本方案把它造出来，连数据一并公开。
...
`ai-traffic-walkability` 228 份（67.5%）、`civic-agent-governance` 221 份（65.4%）、`enterprise-services-ecosystem` 200 份（59.2%）；而 `robotics-autonomous-mobility` 仅 **14 份（4.1%）**、`youth-friendly-public-space` 28 份、`ai-public-services` **30 份（8.9%）**。

三次实测之间这两条赛道持续变厚——机器人 6 → 12 → **14**，公共服务 19 → 26 → **30**——但相对位置始终未变：机器人仍是八条中最薄的一条，且不足次薄者的一半。措辞也随数据回调：上一版因公共服务（26）与青年友好（25）仅差一份而写成「并列次薄」，本次实测为 30 对 28，公共服务重新单独居于次薄位。**一个会随下一次测量改变的排序，就不该写成固定结论。**
...
**结论：任何人都无法从这个字段聚合出「这次征集由哪些模型生成」。** 字段填了，但不可机读——这比「有人留空」更值得修，而且不涉及任何作者的过失。这就是闭合差：出发时声明的规则（结构化、可机读、可被筛选排序），与回到原点时的实测状态（30.2% 空白 + 109 种写法），对不上。

**这条治理回路现在可以闭合了，但还没有闭合——而这个差别本身可以测。** 修法很轻：把 `model` 收敛为枚举 + 自由文本备注两个字段，在四项 gate 中加一条枚举校验。该建议连同全部普查数据与脚本已写入配套 Issue #840，并由 @147228 在 #848 中实现、于 2026-08-09 合入上游。**因此「无法聚合」这一诊断，从 2026-08-09 起不再是机制缺陷，而变成了采用率问题**：实测同日仓库 312 份 `agent.json` 中，声明 `model_family` 的有 **6 份（1.9%）**——字段是可选的，而存量方案都早于它。本方案已在本次提交中采用该字段，但这不改变结论的方向：**一个新增的枚举字段不会自动追溯已提交的三百份包，回路要真正闭合，还需要一次迁移或一次强制。** 这正是本方案关于「限差只可收紧」的同一条道理——规则改了，不等于读数变了。本方案已将该建议与全部普查数据、脚本一并写入配套 Issue。
...
任务书给出三大定位与五大功能 [source:AGENT-TASKBOOK]。本方案不复述，而是把它们接成一条**可以闭合的回路**——这正是多数并行方案的收敛盲点：本次实测显示 338 份中 50.6% 采用「三核」结构、39.6% 采用「两翼」、30.2% 采用「一脊」[source:FIELD-CENSUS-2026-08]。这种高度收敛并非共识，而是任务书本身规定了「三区两翼」所诱导的结果。把同一结构再画一遍没有增量；有增量的是说清楚这些单元之间**靠什么机制传递责任**。
...
**第三行值得单独说，因为它两次改写了本方案自己的发现。** 最初本方案手读两条赛道的 18 份方案，得出「给出数字时限的只有 1 份」，并把「时限」列为自己的增量。这个结论此后被修正了两次：**第一次**，《暂行办法》第十五条早已要求公布反馈时限——所以那从来不是本方案的发明；**第二次**，把手读换成可重跑扫描后（现 36 份，见 `visual/assets/track_scan.json`），命中为 **5 份**，除本方案外 **4 份**——所以「只有 1 份」这个数本身就是错的。 因此真正的发现不是「本方案想到了加时限」，而是——
...
**三项经受住了检验，几项没有。** 冰雪、噪声口径、车队上限三项在除本方案外的 35 份里确为 0；而「申诉时限只有 1 份」「保险只有 4 份」「轮椅通过 0 份」这三处旧表述都低估了同场其他方案，已按实测改正。**关键词正则会漏计同义表述，因此计数是「用过这些词的份数」，不是「想过这件事的份数」**——脚本逐项列出命中名单，正是为了让评审复核而不是采信计数。下表左列标注了该测项在这批方案中的出现情况。
...
**保险与撤除保证金——让退出真的能执行**。两条赛道 36 份中有 9 份提到保证金或保险（除本方案外 8 份），但多为清单中的一个词，未见对风险转移的设计。而本方案的核心规则是「超限即整段退回复测、设备撤除」——这条规则若无资金安排，实际执行时会被推迟成无限期观察。因此：
...
**为什么这两条赛道最需要这台仪器**：338 份实测显示它们分别只有 14 份与 30 份覆盖 [source:FIELD-CENSUS-2026-08]。这不是因为它们不重要，而是因为它们**难写**——一旦认真处理，就必须面对安全、资质、隐私、无障碍与申诉，无法停留在概念层。本方案选择正面处理，并把处理结果写成可被专业团队逐条否证的形式。
...
因此机制被实际跑了一遍。八个用例，每个对应一条规则，全部送进**随包的那个读取器本身**（以子进程调用 `check_closure.js`，不是复制一份逻辑），比对预期与实际：
...
**8/8 与规范一致：2 接受、6 拒绝。** 证据随包提交为 `visual/assets/s08-tabletop-evidence.json`，可重跑。
...
控规深度要求用地覆盖场地而非在场地上点几处功能。本包的 `land_use.geojson` 因此是**总体设计范围的完整无重叠剖分**：五类功能用地按优先级顺序逐一裁剪并做差集，余量再按现状干道切分为街区，全部由脚本确定性生成。

**「可验证」不是形容词，是一组数**。当前 **16 个要素**，在 EPSG:4548 下实测：
...
两条赛道的方案中无人处理设备的充电与停放**选址**（可检索到的提及是标准化充电接口与广场取件点，未涉及选址）。这是一处实际会卡住试点的空缺：一支设备队伍需要充电位、待命位与检修位，三者都占用路缘或地面空间，且都在人流密集处。
...
建筑基底为示意性落位，用于说明功能、规模量级**与朝向**，不构成建筑方案 [depth:retain_renovate_demolish]。**朝向这一项是补上的**：此前六个基底全是缓冲圆——65 顶点的圆盘，看起来像几何，实际不表达任何朝向。而本方案的界面连续性、底层公共性、货运出入口不得开向主脊三条规则，说的都是建筑**怎样面向主脊**，圆形基底一条也承载不了。现改为长边平行于主脊、并向一侧退让 14 m 的矩形，面积量级不变（合计 82,276 m²，原 82,413 m²）。实测两两重叠 **0.0 m²**，压占主脊与附合路线中心线 **0.0 m**——**形状变了，规模声明没变，多出来的是它面朝哪边、以及它站在轴的旁边而不是轴上。**

**后一件事本来是错的，而且错得很难看。** 独立审计发现：改形之前，六个圆形基底中有四个把主脊中心线**吞在里面**，合计 1,618 m。本方案以「连续可步行公共轴」立名，而建筑坐在那条轴上——本包有一条「两个建筑不得共用地面」的硬闸门，却**从来没有一条「建筑不得压占轴线」**，所以它一直没被发现。该闸门现已补上：任一基底与 ROAD-001/002/003 中心线相交长度超过 1 m 即构建失败。退让侧不是随手选的，是由这道闸门筛出来的——**它们被闸门按住，不靠任何人记得。**拆改留分类原则：铁路遗产构筑物原则留；权属清晰且结构安全的既有建筑优先改；无权属争议的低效空地优先用于测点与公共空间。
...
最后一行需要展开说明，因为它涉及一个普遍做法。**人字形折返是京张铁路最具辨识度的符号**，本次征集中有 27 份方案以它为核心元符号 [source:FIELD-CENSUS-2026-08]。作为**叙事符号**引用它完全成立——它属于这条线路的整体历史。但它是位于八达岭一带的**具体工程构筑物，不在本次 43.6 km² 的设计范围内**。因此本方案在空间设计中不使用它，也不把它画进任何图层；本方案取的是同一条线路的另一项遗产——**测量方法**，而测量恰恰贯穿全线，包括范围内的每一米。
...
**绿地为什么有两个数——这是口径不是矛盾。** 独立审计指出 `green_space.geojson` 与 `land_use.geojson` 对同一条绿廊相差 **910,890 m²**，而 `metrics.json` 只发布了大的那个。核实后关系是确定的：完整绿廊 2,311,006.7 m²，其中 910,890 m² 穿过三处重点区（LU-002 内 488,533 m²、LU-003 内 209,080 m²、LU-004 内 155,671 m²、LU-001 内 57,605 m²）；用地剖分把地块归给**主导用途**，因此剖分中的 1401 只剩 1,400,117.1 m²。

两个数都对，缺的是说明。**现两值一并发布，且都由 `verify.js` 独立重算**：从绿地图层得 0.2025，从用地图层得 0.1227。此前只发布前者，评审若从用地图层复算会得到另一个数，且无从判断哪个作数——**一个可能得出两个答案的重算，不是重算。**
